    ...
    static RequireAuth(req, res, next) {
        try {
            ...
            if (!token) {
                return res.status(401).json({
                    success: false,
                    message: guard_error_1.GuardError.MISSING_TOKEN_ERROR
                });
            }
            ...
            if (!decoded) {
                return res.status(401).json({
                    success: false,
                    message: guard_error_1.GuardError.WRONG_ACCESS_TOKEN_INFORMATION
                });
            }
            req.user = decoded;
            next();
            ...
            if (error.name === 'TokenExpiredError') {
                return res.status(401).json({
                    success: false,
                    message: guard_error_1.GuardError.TOKEN_IS_EXPIRED
                });
            }
            return res.status(401).json({
                success: false,
