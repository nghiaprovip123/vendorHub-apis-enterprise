...
const replace_img_helper_utils_1 = require("@/common/utils/replace-img-helper.utils");
const updateStaffService = async (input) => {
    let avatar_url;
    if (input.avatar) {
        const file = await input.avatar;
        const stream = file.createReadStream();
        const replaceImage = await (0, replace_img_helper_utils_1.replaceImageOnCloudinary)(stream, "Staff Avatar Storage", "rqvosutgecru7tutqcdu", true);
